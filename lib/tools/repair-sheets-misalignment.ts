import { getSheetValues, updateRow } from "../adapters/sheets";
...
export type RepairSheetSummary = {
  sheetName: string;
  scanned: number;
  aligned: number;
  repaired: number;
...
async function repairSheet<T extends Record<string, string>>(
  schema: SheetSchema<T>,
  apply: boolean,
  limit: number
): Promise<RepairSheetSummary> {
  ...
  return { sheetName: schema.sheetName, scanned, aligned, repaired, skipped, applied: apply, limited };
}
...
export async function repairSheets(options: RepairOptions): Promise<RepairSheetSummary[]> {
  const apply = Boolean(options.apply);
  const limit = Number.isFinite(options.limit as number)
    ? Math.max(1, Number(options.limit))
    : Number.POSITIVE_INFINITY;
  const sheet = (options.sheet || "").trim();

  const selected = sheet ? schemas.filter(s => s.sheetName === sheet) : schemas;
  if (!selected.length) {
    throw new Error(`Unknown sheet "${sheet}". expected one of: ${schemas.map(s => s.sheetName).join(", ")}`);
  }

  const summaries: RepairSheetSummary[] = [];
  for (const schema of selected) {
    summaries.push(await repairSheet(schema, apply, limit));
  }
  return summaries;
}
