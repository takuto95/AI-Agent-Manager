# Life AI Agent — 統合版最小実装

LINE Webhook、DeepSeek解析、Google Sheets DB、朝/夜のCron通知、ゴール承認フローを1つのNext.js (App Router) プロジェクトにまとめた完全な最小構成です。Cursor / Vercel にそのまま投入すれば動作します。

## 1. ファイル構成
```
/app/api/line/webhook/route.ts   # LINE受信→GoalIntakeサービス
/app/api/line/postback/route.ts  # ゴール承認postback
/app/api/line/push/route.ts      # 管理者向けpush API
/app/api/jobs/morning/route.ts   # 朝のCron通知 (GET/POST)
/app/api/jobs/night/route.ts     # 夜の確認通知
/app/api/jobs/weekly/route.ts    # 週次レビュー通知
/app/api/goals/route.ts          # Goal API (GET/POST)
/app/api/tasks/route.ts          # Task API (GET/POST)
/app/api/progress/route.ts       # Progress API (GET)
/lib/adapters/*.ts               # DeepSeek / LINE / Sheets アダプタ
/lib/core/*.ts                   # GoalIntake / TaskPlanner / Reflectionサービス
/lib/storage/*.ts                # Repositoryインタフェース & Sheets実装
/lib/prompts.ts                  # 鬼コーチ人格/プロンプト群
/vercel.json                     # Vercel Cron設定
.env                             # ローカル環境変数テンプレ
package.json
next.config.mjs
README.md
```

## 2. 必須環境変数 (.env)
```
DEEPSEEK_API_KEY=xxx
SYSTEM_PROMPT=（未指定なら lib/prompts.ts の鬼コーチ人格を使用）
LINE_CHANNEL_ACCESS_TOKEN=xxx
LINE_CHANNEL_SECRET=xxx
LINE_USER_ID=Uxxxxxxxxxxxxxxxxxxxxxxxx
GOOGLE_CLIENT_EMAIL=xxx@appspot.gserviceaccount.com
GOOGLE_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----\n"
SHEETS_SPREADSHEET_ID=1xxxxxxxxxx
BASE_URL=https://your-vercel-url.vercel.app
```
- `GOOGLE_PRIVATE_KEY` の改行は `\n` で表現してください (Vercelでも同様)。
- `LINE_USER_ID` は Cron の push 送信用に固定IDを入れておきます。

## 3. Google Sheets スキーマ
1つのスプレッドシートに **4シート**（`goals / tasks / logs / sessions`）を作成します。

### goals
| A:id | B:title | C:confidence | D:status | E:createdAt | F:updatedAt |

### tasks
| A:id | B:goalId | C:description | D:status | E:dueDate | F:priority | G:assignedAt | H:sourceLogId |

### logs
| A:id | B:timestamp | C:userId | D:rawText | E:emotion | F:coreIssue | G:currentGoal | H:todayTask | I:warning |

### sessions
| A:sessionId | B:userId | C:type | D:content | E:timestamp | F:meta |

## 4. DeepSeek/LINE/Sheets 連携の流れ
1. ユーザーがLINEで送信 → `app/api/line/webhook/route.ts` がセッション（`sessions` シート）を更新しつつ応答。
2. `#整理開始 ... #整理終了` のあと `#タスク整理` を送ると、終了済みログの transcript を DeepSeek に投入して JSON を抽出し、`logs` と `tasks` を追記。
3. ゴール承認は **postbackのみ**対応（`approve_goal:<内容>`）。`app/api/line/postback/route.ts` 経由で `goals` に追加される（現状 status は `pending` 固定）。
4. Cron `/api/jobs/morning` が `tasks` から次の `todo` を拾って命令文を push。
6. `/api/jobs/night` は必要に応じて手動 or 外部 scheduler で叩き、進捗確認を push。
7. Cron `/api/jobs/weekly` が直近7日のログを DeepSeek でレビューして push。

## 5. vercel.json (Cron)
```json
{
  "crons": [
    { "path": "/api/jobs/morning", "schedule": "0 21 * * *" },
    { "path": "/api/jobs/weekly", "schedule": "0 12 * * 0" }
  ]
}
```
Vercel無料枠では Cron Job が2件までなので、ここでは「朝」「週次」を常時動かし、夜は手動／外部トリガーで運用する想定です。時刻はUTC基準なので、JST朝6時/週次日曜21時に合わせて 21時/12時(UTC) を指定しています。必要に応じて `schedule` を調整してください。

## 6. セットアップ & デプロイ
1. `npm install`（または `pnpm install`）で依存を導入。
2. `.env` を作成し、上記の環境変数を入力。ローカル検証は `npm run dev`、Vercelローカルは `npm run vercel:dev`。
3. Google Cloud Console でサービスアカウントを作成し、Sheets API を有効化 → メール／秘密鍵を `.env` に設定。
4. Sheets に `goals / tasks / logs / sessions` シートを作成し、1行目をヘッダーにする（空行があると `slice(1)` でズレるので注意）。
5. LINE公式アカウントで Messaging API を有効化し、Webhook URL を `https://<vercel-host>/api/line/webhook` に設定 → 接続確認を ON。
6. リポジトリをGitHubへ push → Vercel プロジェクトを接続し、同じ環境変数を「Environment Variables」に登録 → Deploy。
7. Vercel Dashboard > Cron Jobs で `/api/jobs/morning` / `/api/jobs/weekly` を確認（`vercel.json` を push 済みなら自動検出）。

## 7. ローカル / 本番テスト手順
1. LINEでBotに「今日は仕事で自信がなかった」などのログを送信。
   - `sessions` シートに user/assistant イベントが追記されていることを確認。
2. `#整理開始` → ログを数回送る → `#整理終了` → `#タスク整理` の順で送る。
   - `logs` に1行、`tasks` に1行以上が追記される（`sourceLogId` で紐付く）。
3. ゴールを採用したい場合は、テンプレボタンなどで postback を送る（`approve_goal:○○`）。`goals` に追加される。
3. `/api/jobs/morning` を叩く（Vercel Cron「Run now」または `curl https://<vercel-host>/api/jobs/morning`）と朝の命令文が届く。
4. `/api/jobs/night` を手動実行（`curl` か外部 scheduler）し、夜の確認メッセージが届くか確かめる。
5. `/api/jobs/weekly` を叩き、直近7日のログが十分なら週次レビューが届く（ログ不足なら警告が返る）。
6. Cron を有効化した後は、Vercel Logs で `morning cron failed` / `weekly endpoint failed` などのエラーがないか監視する。

## 8. コスト抑制の実装ポイント
- DeepSeekへは生ログではなく短い要約プロンプトのみを送信 (コードでJSONテンプレのみ送信)。
- 朝のCronは軽量プロンプトで命令文整形のみ、週次で重い解析をする場合は別関数を作成。
- Sheetsは無料枠で十分。レコード増加時はエクスポートまたは将来Supabaseへ移行。

## 9. 監視・ロギング
- VercelのFunction Logsで `/api/line/webhook` / Cron のエラーを確認。
- DeepSeek/Sheets失敗時はLINEにフォールバック文言を返信する（`app/api/line/webhook/route.ts`）。
- Google Sheetsの行数が増えたら月次でバックアップ (Apps ScriptやCSVエクスポートなど)。

## 10. オプション
- **フルコードパッケージ**: 本リポジトリで提供。
- **プロンプト群**: `lib/prompts.ts` に基本テンプレを格納。必要に応じて朝/夜/週次を追記できます。
- **リッチメニュー/テンプレ**: `lib/adapters/line.ts` のユーティリティを使えばボタン承認に拡張可能です。

このREADMEの手順に沿って環境変数を設定すれば、即座にLINE×DeepSeek×Sheets連携の行動マネージャーを稼働できます。

## 11. 変更履歴

### 2024-12-30: タスク更新失敗の検知と通知機能を追加

**問題:**
- ユーザーがタスクを報告したのに完了状態にならない不具合が発生

**原因:**
- `done`/`miss` コマンド実行時に `updateStatus` の戻り値を確認していなかった
- Google Sheets API エラーが発生しても成功メッセージを返してしまう
- 更新が実際に成功したか検証していなかった

**対策:**
- ✅ `updateStatus` の戻り値を確認し、失敗時は詳細なエラーメッセージを返す
- ✅ try-catch で Sheets API エラーを個別に捕捉
- ✅ 更新後の状態を確認して整合性を保証
- ✅ 詳細なログを出力してトラブルシューティングを容易に
- ✅ 新コマンド `status <taskId>` を追加（タスクの現在の状態を確認可能）

**影響範囲:**
- `app/api/line/webhook/route.ts`: done/miss/夜報告の処理を修正
- `docs/bmad/spec-current.md`: 更新検証機能を追記
- `docs/bmad/gaps.md`: 完了事項として記録
- `docs/bmad/fix-2024-12-30-task-update-verification.md`: 詳細な修正ドキュメント

**互換性:**
- 破壊的変更なし
- エラーメッセージがより詳細になる（改善）

### 2024-12-30: 日報UX改善（入力形式の明確化とエラー検知）

**問題:**
- ユーザーが `1 done` と入力したが認識されなかった（`done 1` のみ対応）
- タスクIDが `t_` から始まるため、`1 done` は番号とIDの混同を招く
- 日報開始時のメッセージが長く、使い方が分かりにくい

**改善内容:**
- ✅ 明確な入力形式: `done 1`（動詞が先）のみを推奨
- ✅ エラー検知: 逆順（`1 done`）を送ると、正しい形式を提示
- ✅ メッセージ分割表示: 日報開始時に3つのメッセージに分割して見やすく
- ✅ 簡潔な応答: 絵文字を使って一目で分かる（✅完了 / ❌未達 / 📝メモ）
- ✅ 新関数 `replyTexts`: 複数メッセージを一度に送信

**影響範囲:**
- `app/api/line/webhook/route.ts`: 入力形式を明確化、エラー検知追加、メッセージ分割
- `lib/adapters/line.ts`: `replyTexts` 関数追加
- `docs/bmad/spec-current.md`: 推奨入力形式とエラー検知を明記
- `docs/bmad/fix-2024-12-30-improved-ux.md`: 詳細な改善ドキュメント

**互換性:**
- ✅ 既存の正しい形式（`done 1`）は引き続き動作
- ⚠️ 逆順（`1 done`）は無効だが、エラーメッセージで修正案を提示

### 2024-12-30: モチベーション向上・継続性改善

**問題:**
- タスク表示が多すぎて見づらい（日報開始時に全件表示）
- メッセージが事務的で、ユーザーがやる気を失っている
- 継続して使いたくなる仕組みがない

**改善内容:**
- ✅ タスク表示を2-3件に制限（優先度の高いもののみ、`list` で全件表示可能）
- ✅ 完了時のランダムな褒め言葉（「よくやった！」「素晴らしい！」など）
- ✅ 未達時の前向きなフィードバック（「大丈夫。次がある。」など）
- ✅ 日報終了時の進捗サマリー（「今日は2件完了！達成率67%」）
- ✅ ストリーク表示（「🔥 連続5日！」）
- ✅ 朝のメッセージ改善（命令口調→励まし口調）
- ✅ 週次レビュー改善（批判→ポジティブフィードバック）

**影響範囲:**
- `app/api/line/webhook/route.ts`: タスク表示制限、モチベーションメッセージ、ストリーク機能
- `lib/prompts.ts`: 朝のメッセージと週次レビュープロンプトを改善
- `lib/core/reflection-service.ts`: 週次レビューのメッセージフォーマット改善
- `docs/bmad/fix-2024-12-30-motivation-improvement.md`: 詳細な改善ドキュメント

**効果:**
- ✅ UIがスッキリして見やすい
- ✅ 小さな成功体験を祝う
- ✅ 前向きに受け止められる
- ✅ 継続のモチベーション
- ✅ メッセージを見たくなる

### 2024-12-30: 思考ログモードの対話的改善（深層心理を探る）

**問題:**
- 単に情報を整理するだけで、対話が浅い
- ユーザーの深層心理を探る質問がない
- 表面的な言葉を受け取って、そのまま返しているだけ

**改善内容:**
- ✅ システムプロンプトを対話型に変更（深層心理の探求）
- ✅ 深掘り質問の原則を追加（「なぜ？」「本当は？」「何が怖い？」）
- ✅ 返信メッセージを対話的に改善（共感→深掘り→気づきを促す）
- ✅ 開始・終了メッセージを柔らかく改善

**対話の特徴:**
- 表面的な言葉の奥を探る（「忙しい」→「本当は何が怖いの？」）
- 思考の矛盾を優しく指摘する
- ユーザー自身が気づくように導く
- 答えを押し付けない

**影響範囲:**
- `lib/prompts.ts`: システムプロンプトと分析プロンプトを対話型に変更
- `app/api/line/webhook/route.ts`: 返信メッセージと開始・終了メッセージを改善
- `docs/bmad/spec-current.md`: 思考ログモードの説明を更新
- `docs/bmad/fix-2024-12-30-thought-log-deep-dive.md`: 詳細な改善ドキュメント

**効果:**
- ✅ 自己理解の深化（自分でも気づいていない本音に気づける）
- ✅ 行動変容（深層心理に気づくことで、真の問題を解決できる）
- ✅ 継続的な利用（深い対話ができるので、継続して使いたくなる）
- ✅ 差別化（単なる整理ツールではなく、深層心理を探る対話型AI）

### 2024-12-30: ゴールとタスクの連携・missタスクフォロー

**問題:**
- ゴールとタスクの関係が見えない（何のためにやっているか分からない）
- missタスクが放置される（次にどうすればいいか分からない）

**改善内容:**
- ✅ ゴール進捗の計算機能（完了数/総数で進捗率を計算）
- ✅ タスク一覧にゴール情報を表示（「→ キャリアアップを実現する」）
- ✅ 日報終了時にゴール進捗を表示（「キャリアアップ: ████░░░░░░ 40%」）
- ✅ miss時に次のアクション提案（再挑戦/分割/延期の選択肢）
- ✅ タスク操作コマンドの準備（split/retry）

**影響範囲:**
- `lib/storage/repositories.ts`: GoalsRepository, TasksRepository にメソッド追加、GoalProgress 型追加
- `lib/storage/sheets-repository.ts`: メソッド実装
- `app/api/line/webhook/route.ts`: ゴール情報表示、miss時の提案、コマンド追加
- `docs/bmad/spec-current.md`: ゴールとタスクの連携を追記
- `docs/bmad/fix-2024-12-30-goal-task-linkage.md`: 詳細な改善ドキュメント

**効果:**
- ✅ モチベーション向上（何のためにやっているのかが見える）
- ✅ 継続性の向上（missタスクが放置されない）
- ✅ 長期的な視点（ゴール全体への貢献が見える）

### 2024-12-30: キーワードレス化（入力負担の削減）

**問題:**
- `#整理開始` `#日報開始` などのキーワードを覚える必要がある
- コマンドが多くて覚えられない
- ユーザーの入力負担が大きい

**改善内容:**
- ✅ 番号でモード選択（1=思考ログ, 2=日報, 3=タスク化）
- ✅ AIが自動でモード判断（「モヤモヤ」→思考ログ自動開始）
- ✅ 「終了」でシンプルに終了
- ✅ LINEクイックリプライボタン（タップするだけ）

**AIが自動判断するキーワード:**
- 思考ログ: モヤモヤ、悩み、考え、迷、不安、困、どうしよう、わからない
- 日報: 報告、完了、未達、done、miss、やった、できた、できなかった

**影響範囲:**
- `app/api/line/webhook/route.ts`: 番号選択、AI自動判断、終了簡略化
- `docs/bmad/spec-current.md`: キーワードレス化を追記
- `docs/bmad/fix-2024-12-30-keywordless-and-improvements.md`: 詳細ドキュメント

**改善内容（追加）:**
- ✅ タスク分割の完全実装（AIが分割案を提案 → 承認で実行）
- ✅ タスク再挑戦の完全実装（missタスクをtodoに戻す）
- ✅ タスク生成時の理由説明（なぜこのタスクが必要か）
- ✅ 週次進捗の比較（先週比、増減率表示）
- ✅ マイルストーン・バッジ（ストリーク・完了件数に応じたバッジ）

**影響範囲:**
- `app/api/line/webhook/route.ts`: タスク分割、タスク再挑戦、バッジ機能追加
- `lib/core/goal-intake-service.ts`: タスク生成時の理由説明
- `lib/core/reflection-service.ts`: 週次進捗の比較
- `lib/storage/repositories.ts`: TaskRecordにreason追加
- `lib/storage/sheets-repository.ts`: reason対応
- `lib/core/types.ts`: AnalysisTaskにreason追加
- `lib/prompts.ts`: タスク生成プロンプトにreason追加
- `docs/bmad/fix-2024-12-30-keywordless-and-improvements.md`: 詳細ドキュメント

**効果:**
- ✅ 入力負担の削減（1文字または自然な言葉でOK）
- ✅ 直感的な操作（番号やボタンでタップ）
- ✅ AIが意図を汲み取る（ストレスフリー）
- ✅ 継続しやすい（操作が簡単だから続けやすい）
- ✅ タスクの理解が深まる（理由説明）
- ✅ 大きなタスクを分割できる（実行可能な粒度に）
- ✅ missタスクをやり直せる（諦めない）
- ✅ 達成感とモチベーション向上（バッジ、先週比）

### 2024-12-30: 月次・四半期レビュー、学習とパーソナライズ

**問題:**
- 長期的な視点でのレビューがない（週次のみ）
- ユーザーの傾向を学習して提案する機能がない
- cronが2つまでしか動かせない制約

**改善内容:**
- ✅ 月次レビュー（週次cronで月末判定して実行）
- ✅ 四半期レビュー（週次cronで四半期末判定して実行）
- ✅ 学習とパーソナライズ（ユーザーの傾向分析と提案）

**月次・四半期レビュー:**
- 週次cronで月末・四半期末を判定（`isEndOfMonth()`, `isEndOfQuarter()`）
- 月末: 30日分のログとタスク実績を振り返り
- 四半期末（3/6/9/12月末）: 90日分のログとタスク実績を振り返り
- cronの制約（2つまで）を考慮し、週次cronに統合

**学習とパーソナライズ:**
- 優先度別の傾向（A/B/Cごとの完了率）
- 曜日別の傾向（日〜土ごとの完了率）
- missタスクのパターン（キーワード、文字数）
- doneタスクのパターン（キーワード、文字数）
- AIが自動で提案を生成:
  - 優先度Aの達成率が低い → 優先度調整
  - missタスクが長い → タスク簡略化
  - 特定の曜日の達成率が低い → 時間調整
  - 全体の達成率が高い → 励まし

**影響範囲:**
- `app/api/jobs/weekly/route.ts`: 月末・四半期末判定、追加レビュー実行
- `lib/core/reflection-service.ts`: 月次・四半期レビューメソッド追加
- `lib/core/learning-service.ts`: 新規作成（傾向分析と提案生成）
- `lib/prompts.ts`: 月次・四半期レビュープロンプト追加
- `app/api/line/webhook/route.ts`: 日報終了時にAIの提案を表示
- `docs/bmad/spec-current.md`: 新機能を追記
- `docs/bmad/fix-2024-12-30-keywordless-and-improvements.md`: 詳細更新

**効果:**
- ✅ 長期的な視点での振り返り（1ヶ月、3ヶ月）
- ✅ ユーザーの傾向を学習して改善提案
- ✅ cronの制約内で実装（週次cronに統合）
- ✅ モチベーション向上（励ましと具体的な提案）
